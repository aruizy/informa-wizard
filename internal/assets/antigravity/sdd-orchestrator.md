...
For this agent (inline execution): **Interactive** is already the default behavior — you already pause between phases. **Automatic** means run all phases sequentially without stopping to ask between them. **Plan-Build** means you pause during planning phases but run build phases sequentially without stopping after the transition confirmation.
