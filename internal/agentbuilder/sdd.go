...
// Example: <!-- informa-wizard:custom-agent:my-skill -->
const markerFormat = "<!-- informa-wizard:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /informa-wizard:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /informa-wizard:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- informa-wizard:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- informa-wizard:custom-agent:"
	suffix := " -->"
