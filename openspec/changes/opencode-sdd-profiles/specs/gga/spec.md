...
- WHEN `informa-wizard sync --profile cheap:anthropic/claude-haiku-3.5-20241022` runs
...
- GIVEN `informa-wizard sync --profile badformat` is run (no colon separator)
...
- GIVEN `informa-wizard sync --profile cheap:haiku --profile-phase cheap:sdd-apply:sonnet`
- WHEN sync runs
