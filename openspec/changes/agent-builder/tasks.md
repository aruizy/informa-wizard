...
- [ ] T-07 Create `internal/agentbuilder/registry.go` — implement `Registry` struct with `Load(path)`, `Save(path)`, `Add(entry)`, `List()` methods; JSON at `~/.config/informa-wizard/custom-agents.json`; `version: 1` field; conflict detection for built-in and custom name collisions
...
- [ ] T-09 Create `internal/agentbuilder/sdd.go` — implement `InjectSDDReference(agent, adapter, homeDir)` injecting `<!-- informa-wizard:custom-agent:{name} -->` marker blocks into agent system prompts; handle phase-support (augment existing) and new-phase (insert into pipeline graph); no duplication on re-run
