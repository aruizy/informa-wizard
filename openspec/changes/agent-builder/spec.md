...
The system MUST read and write a JSON registry at `~/.config/informa-wizard/custom-agents.json`. The registry MUST include a `version` field (integer). Each entry MUST record: name, title, description, created_at (RFC3339), generation_engine, sdd_integration (nullable), installed_agents.
...
For `phase-support` mode, the installer MUST inject a marker-fenced block into the agent's system prompt file using the `StrategyMarkdownSections` approach. The marker MUST follow the format `<!-- informa-wizard:custom-agent:{name} -->`.
...
- GIVEN the system prompt already contains `<!-- informa-wizard:custom-agent:a11y-reviewer -->`
