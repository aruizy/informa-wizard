...
When `informa-wizard` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `informa-wizard install` is idempotent.
...
`informa-wizard install`**. In the final design this was moved to **runtime** — the `.ps1`
